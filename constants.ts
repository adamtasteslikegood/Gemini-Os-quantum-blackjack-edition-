...
export const APP_DEFINITIONS_CONFIG: AppDefinition[] = [
  ...
  {id: 'quantum_monte_app', name: 'Quantum Monte', icon: '⚛️', color: '#e8eaf6'},
];
...
0.  **Available apps:** The computer has several apps that can be opened from home screen.
    ...
    - "Quantum Monte": A "Find the Queen" card game with a quantum twist.
      - **Initial State:** When the app is opened, display a title like "Quantum Three Card Monte" and a "Start Game" button with \`data-interaction-id="qmc_start_game"\`. Explain that a Queen of Hearts and two random non-Queen cards will be used.
      - **Superposition State:** After the user clicks "Start Game", show three interactive face-down cards (e.g., using styled divs with a '🃏' emoji). Add text prompting the user to "Choose a card to collapse the quantum state!". Each card must have a unique ID, e.g., \`data-interaction-id="qmc_choose_card_1"\`, \`data-interaction-id="qmc_choose_card_2"\`, \`data-interaction-id="qmc_choose_card_3"\`.
      - **Collapse State (Result):** When the user clicks a card (e.g., \`qmc_choose_card_2"\`), you must *randomly* determine the outcome.
          - **Randomly decide the winning card position (1, 2, or 3).** This is critical.
          - Compare the user's chosen card position to the winning position.
          - Reveal all three cards. Use card emojis like '❤️Q' for the Queen of Hearts, and other random cards like '♠️7', '♦️3' for the others. The other two cards must not be Queens and should be different from each other.
          - Announce clearly whether the user won or lost.
          - Provide a "Play Again" button with \`data-interaction-id="quantum_monte_app"\` to restart the game.
